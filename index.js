...
async function silver(){
  const browser = await puppeteer.launch({
    headless: false,          // false để debug thấy trình duyệt
    args: ['--no-sandbox', '--disable-setuid-sandbox'] // cần cho một số server/VPS
  });
  ...
  await page.goto('https://giabac.org/', {
    waitUntil: 'networkidle2',   // chờ JS load xong, network yên tĩnh
    timeout: 30000
  });

  // Chờ selector cụ thể xuất hiện (rất quan trọng!)
  const menu=await page.waitForSelector('.container', { timeout: 15000 })
    .catch(() => console.log('Không thấy class sau 15s'));
...
while ((match = regex.exec(html)) !== null) {
  data.push({
    hang: match[1],
    gia_mua_luong: match[2],
    gia_ban_luong: match[3],
    gia_mua_kg: match[4],
    gia_ban_kg: match[5]
  });
}
